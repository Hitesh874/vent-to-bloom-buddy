...
import { Leaf, TreePine, Sprout, Award, Share2, Download, Home } from 'lucide-react';
...
const Garden = () => {
  ...
  const getTreeIcon = (type: string) => {
    switch (type) {
      case 'mature': return <TreePine className="w-12 h-12 text-green-700" />;
      case 'young': return <TreePine className="w-10 h-10 text-green-600" />;
      case 'sapling': return <Sprout className="w-8 h-8 text-green-500" />;
      default: return <Leaf className="w-6 h-6 text-green-400" />;
    }
  };

  const getTreeHeight = (type: string) => {
    switch (type) {
      case 'mature': return 'h-32';
      case 'young': return 'h-24';
      case 'sapling': return 'h-16';
      default: return 'h-8';
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50">
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="absolute bottom-8 left-0 w-full flex items-end justify-center space-x-8">
              {trees.map((tree, index) => (
                <div key={tree.id} className="flex flex-col items-center group cursor-pointer">
                  <div className={`${getTreeHeight(tree.type)} flex items-end justify-center transition-transform group-hover:scale-110`}>
                    {getTreeIcon(tree.type)}
                  </div>
                  <div className="mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Badge className="bg-white/90 text-green-800 text-xs">
                      Tree #{index + 1}
                    </Badge>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center opacity-50">
                ...
                <Badge className="mt-2 bg-green-100 text-green-600 text-xs">
                  3 vents to go!
                </Badge>
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {trees.map((tree, index) => (
            <Card key={tree.id} className="p-6 bg-white/70 backdrop-blur-sm border-green-100 hover:shadow-lg transition-shadow">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <Badge className={`${
                      tree.type === 'mature' ? 'bg-green-100 text-green-800' :
                      tree.type === 'young' ? 'bg-blue-100 text-blue-800' :
              ...
              {tree.type === 'sapling' && (
                <div className="mt-4 p-3 bg-green-50 rounded-lg">
                  ...
                  <div className="text-xs text-green-600 mt-1">
                    {10 - tree.ventCount} more vents to mature
                  </div>
                </div>
              )}
            </Card>
